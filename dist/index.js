...
// 電話番号を抽出して検証する関数
function extractAndValidatePhoneNumber(text) {
    // 電話番号らしき文字列を探す
    const phoneMatch = text.match(/0[0-9-０-９]+/);
    if (!phoneMatch)
        return { isValid: false };
    // 全角数字を半角に変換
    const normalized = phoneMatch[0].replace(/[０-９]/g, (s) => String.fromCharCode(s.charCodeAt(0) - 0xfee0));
...
async function handleEvent(event) {
    ...
    // 電話番号の処理
        ...
        const { isValid, number, hasInvalidChars } = extractAndValidatePhoneNumber(text);
        if (isValid && number) {
...
// 確認メッセージの表示
async function showConfirmation(userId) {
...
// メール本文のフォーマット
function formatMailBody(userId) {
...
// 無効な電話番号の処理
async function handleInvalidPhoneNumber(userId) {
    await lineClient.pushMessage(userId, {
        type: "text",
...
// フォーマットガイドの表示
async function handleFormatGuide(userId) {
    await lineClient.pushMessage(userId, {
        type: "text",
        text: "以下の形式で入力してください：\n\n【お名前】山田太郎\n【電話番号】090-1234-5678",
    });
